...
from alg.FedLocalScoreFunctionClass import LocalScoreClass
...
def ges(X: ndarray, score_func: str = 'local_score_BIC', maxP: Optional[float] = None,
        parameters: Optional[Dict[str, Any]] = None,client_num=10,is_iid=True,dropout=0) -> Dict[str, Any]:
    """
    ...
    if score_func == 'local_score_CV_general':  # % k-fold negative cross validated likelihood based on regression in RKHS
        ...
        localScoreClass = LocalScoreClass(data=X, local_score_fun=local_score_cv_general, parameters=parameters,client_num=client_num)
        ...
        if maxP is None:
            maxP = X.shape[1] / 2  # maximum number of parents
        N = X.shape[1]  # number of variables
        localScoreClass = LocalScoreClass(data=X, local_score_fun=local_score_marginal_general, parameters=parameters,client_num=client_num)
        ...
        localScoreClass = LocalScoreClass(data=X, local_score_fun=local_score_cv_multi, parameters=parameters,client_num=client_num)
        ...
        if maxP is None:
            maxP = len(parameters['dlabel']) / 2
        N = len(parameters['dlabel'])
        localScoreClass = LocalScoreClass(data=X, local_score_fun=local_score_marginal_multi, parameters=parameters,client_num=client_num)
        ...
        localScoreClass = LocalScoreClass(data=X, local_score_fun=local_score_BIC, parameters=parameters,client_num=client_num,dropout=dropout)

    elif score_func == 'local_score_BDeu': 
        if is_iid: # Greedy equivalence search with BDeu score
            if maxP is None:
                maxP = X.shape[1] / 2
            N = X.shape[1]  # number of variables
        else:
            if maxP is None:
                maxP = (X.shape[1]-1) / 2
            N = X.shape[1]-1  # number of variables
        localScoreClass = LocalScoreClass(data=X, local_score_fun=local_score_BDeu, parameters=None,client_num=client_num,is_iid=is_iid,dropout=0)
